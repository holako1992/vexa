...
`components/CalendarOAuthCallback` with `provider="google"` inside `Shell`, the same split every
other route in `src/app/` uses. No nav-rail entry — reached only from Google's own redirect, the
same way `/search` is reached only from the search box.

`CalendarOAuthCallback` is the ONE component both this page and `../../microsoft/callback/page.tsx`
