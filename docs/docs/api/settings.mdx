...
Two more top-level fields cover a support comp — raising a user above (or setting them aside
from) their billed plan without touching Stripe: see
[Support comps (admin overrides)](/how-to/billing#support-comps-admin-overrides) for the full
contract. In short:
...
- `plan_override` — a catalog plan id (`free`/`pro`/`team`), or `null` to clear it. An
  unrecognized id is **422**.
- `quota_bonus` — a non-negative integer, extra meetings for the CURRENT resolved billing period
  only (never carried into the next one); `null` clears it. A negative value is **422**.
