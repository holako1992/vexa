...
to page explicitly). Both `GET /bots` and `GET /meetings` return `has_more: true` when more rows
remain past the current page — page by incrementing `offset` until it comes back `false`.
