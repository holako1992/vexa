...
| `max_bot_time_exceeded` | The run hit its hard lifetime cap and was ended. | **Your policy, or your plan** | Raise `automatic_leave.max_bot_time` if your meetings run longer — but see [the per-plan minute cap](#the-per-plan-minute-cap) below: your plan's own per-meeting minute allowance narrows whatever you ask for, and only an upgrade raises THAT. The transcript up to the cap is kept. |
...
### The per-plan minute cap

`max_bot_time` is a ceiling you can LOWER freely, never one you can raise past your plan. Each plan
in the catalog (`billing/catalog.py`) carries its own per-meeting minute allowance — Free 60
minutes, Pro and Team 240 — and the bot's actual cap is the SHORTER of that plan allowance and
whatever `max_bot_time` you pass (missing either one just leaves the other in force). A Free-plan
request for `automatic_leave: {"max_bot_time": 7200000}` (2h) still ends at 60 minutes with
`max_bot_time_exceeded` — the request was accepted, the number asked for was simply above what the
plan allows. [Upgrade](/how-to/billing) to raise the allowance itself; see [Read the resolved
plan](/how-to/billing#read-the-resolved-plan) for `max_minutes_per_meeting` on your own account.
